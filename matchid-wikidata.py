...
from pprint import (
    pprint,
)  # nécessaire pour pprint (affichage des données JSON sur plusieurs lignes)
...
try:
    ...
    if len(date_debut) != 10:
        raise SystemExit("Erreur : Renseigner une date de début de recherche")
...
try:
    ...
    if len(date_fin) != 10:
        raise SystemExit("Erreur : Renseigner une date de fin de recherche")
...
try:
    ...
    try:
        ...
        print(
            "Pas de fichier apikey.txt contenant la clé API (vous pouvez en obtenir une sur le site deces.matchid.io) trouvé"
        )
        apikey = input(
            "Renseignez manuellement l'API Key si vous en avez une (laisser vide sinon, mais le nombre de demandes va être limité) : "
        )

endpoint_url = "https://query.wikidata.org/sparql"

query = (
    """SELECT ?human ?humanLabel ?nom_de_familleLabel ?prenomLabel ?prenoms_num ?prenomsLabel ?nom_de_naissance ?pseudo ?date_de_naissance ?precision_de_naissance ?lieu_de_naissanceLabel ?date_de_mort ?precision_de_mort ?lieu_de_mortLabel WHERE {
  ?human wdt:P31 wd:Q5.
...
for i in range(len(results["results"]["bindings"])):
    ...
    for j in range(len(item_wikidata)):
        try:
            wikidata[i].append(
                results["results"]["bindings"][i][item_wikidata[j]]["value"]
            )

            if item_wikidata[j] == "human":
                wikidata[i][j] = wikidata[i][j][
                    31:
                ]  # pour enlever "http://www.wikidata.org/entity/" de "http://www.wikidata.org/entity/Q55740039"

            if item_wikidata[j] == "date_de_naissance":
                wikidata[i][j] = wikidata[i][j][
                    :10
                ]  # pour garder "2010-02-02" dans "2010-02-02T00:00:00Z"
                d = date.fromisoformat(wikidata[i][j])
                wikidata[i][j] = d.strftime("%d/%m/%Y")  # pour passer à JJ/MM/AAAA
                if results["results"]["bindings"][i]["precision_de_naissance"][
                    "value"
                ] in {
                    "7",
                    "8",
                }:  # précision = siècle ou décennie
                    wikidata[i][j] = ""
                if (
                    results["results"]["bindings"][i]["precision_de_naissance"]["value"]
                    == "9"
                ):
            ...
            if item_wikidata[j] == "date_de_mort":
                ...
                d = date.fromisoformat(wikidata[i][j])
                wikidata[i][j] = d.strftime("%d/%m/%Y")  # pour passer à JJ/MM/AAAA
                if results["results"]["bindings"][i]["precision_de_mort"]["value"] in {
                    "7",
                    "8",
                }:  # précision = siècle ou décennie
                    wikidata[i][j] = ""
                if (
                    results["results"]["bindings"][i]["precision_de_mort"]["value"]
                    == "9"
                ):
                    wikidata[i][j] = d.strftime("%Y")  # pour passer à AAAA
        ...
        print(
            ...
            "  ",
            item_wikidata[j],
            (30 - len(item_wikidata[j]) - len(str(i)) - len(str(j))) * " ",
            " : ",
            wikidata[i][j],
        )
...
for i in range(len(wikidata)):
    print(
        f"----- i: {i}  /  {len(wikidata)} ({round(i/len(wikidata)*100,1)}%) (ETA: {round(gap*nb_methodes*(len(wikidata)-i)/60*1.25,1)} min)  -----  {wikidata[i][0]}   {wikidata[i][1]}"
    )
    pprint(wikidata[i])
    if (
        1
    ):  # wikidata[i][0] != human_precedent: #pour trouver chaque nouveau human (Qxxxxxxx)
        res_saved.append([])

        params_matrice = [  # Nom DOD
            {
        ...
        for m in range(len(params_matrice)):  # balaye chaque ligne de la matrice
            ...
            try:
                print(
                    "PROBLEME DE REPONSE =========================================== Message retourné par deces.matchid.io :",
                    res["message"],
                )
            except:
                pass
            ...
            if response_data is not None:
                ...
                if (
                    scored_size == 0
                ):  # si score 0 mais qu'il y a eu des réponses, on retourne la 1ère quand même
                    for o in range(min(response_data["total"], 1)):
                        if response_data["persons"][o]["score"] is not None:
                            response_data["persons"][o][
                                "score"
                            ] = 0.001  # on gonfle un peu le score, sinon le template jinja va masquer la ligne
                            res_saved[k][m].append(response_data["persons"][o])
                            print(
                                "i: ",
                                i,
                                ...
                                "  m:",
                                m,
                                "  on affiche quand même le 1er résultat avec un score 0.001",
                            )
        k = k + 1
    else:
...
rendered = env.get_template("template.html").render(
    ...
    date_fin=date_fin,
    wikidata=wikidata,
    len_wikidata=len(wikidata),
    item_wikidata=item_wikidata,
    len_item_wikidata=len(item_wikidata),
    res_saved=res_saved,
)
